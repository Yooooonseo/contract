...
public class ContractService {
    ...
    /*private Map<String, String> createFieldMappingVenture() {
        Map<String, String> fieldMapping = new HashMap<>();
        //fieldMapping.put("investorName1", "dhFormfield-5193825025");
        fieldMapping.put("ventureName1", "dhFormfield-5193704257");
        //fieldMapping.put("price", "dhFormfield-5193831145");
    ...
    */
    ...
    // PDF 필드에 데이터 채우기
   /* private void fillPdfFields(Map<String, PdfFormField> fields, Map<String, String> fieldData) {
        for (Map.Entry<String, String> entry : fieldData.entrySet()) {
    ...
    // PDF 계약서 생성 로직 (기업과 투자자 데이터 모두 포함)
    /*private void createContract(String outputFilePath, Map<String, String> fieldData) throws IOException {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfReader("path/to/template.pdf"), new PdfWriter(outputFilePath))) {
            PdfAcroForm form = PdfAcroForm.getAcroForm(pdfDoc, true);

            // PDF 필드 매핑 및 계약서 데이터 채우기
            Map<String, PdfFormField> fields = form.getFormFields();
            fillPdfFields(fields, fieldData);
